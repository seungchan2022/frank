...
| M2 | 버그 수정 — 키워드 오염 + 에러 메시지 개선 | 1~2일 | 완료 |
| M3 | 피드 성능 — 서버 TTL 캐시 도입 | 2~3일 | 완료 |
...
| 260414 | M2 완료 — keyword_weights tag_id 분리, 요약 에러 메시지 개선, 서버/웹/iOS 전층 수정 | 실사용 테스트 통과 |
| 260414 | M3 완료 — 서버 TTL 인메모리 캐시(5분), 웹/iOS no-cache 헤더, 태그 변경 시 캐시 무효화 | 서버 285 + 웹 221 + iOS 전체 테스트 통과 |
| 260414 | MVP10 통합 테스트 완료 — 서버 로그로 MISS→HIT→BYPASS(no-cache) + 사용자 격리 동작 확인 | 웹·iOS 시나리오 모두 실제 동작 검증 |
