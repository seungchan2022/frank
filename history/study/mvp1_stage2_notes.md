...
**④ auth.isAuthenticated 확인 ($effect) — 브라우저 보조 가드**

마운트 직후 `$effect`가 먼저 실행돼. `auth.isAuthenticated`가 false면 `/login`으로 보내는 보조 가드 역할이야.

여기서 **클라이언트 = 브라우저에서 실행되는 코드**야. 웹 앱은 코드가 두 곳에서 실행돼. 서버 컴퓨터에서 실행되는 코드(+layout.server.ts)와 유저 브라우저에서 실행되는 코드($effect, onMount). [A]의 인증 가드는 서버에서, 여기 ④의 가드는 브라우저에서 동작해.

[A]에서 서버가 이미 인증 체크를 했는데 왜 또 하냐면, 역할이 달라. [A]는 "페이지 진입 시점에 딱 한 번" 검증이고, `$effect`는 "페이지가 열려있는 내내 상태 변화를 감시"해. 온보딩 페이지를 열어두고 30분 방치 → 토큰 만료, 또는 다른 탭에서 로그아웃하면 [A] 가드는 이미 통과했으니 다시 실행 안 돼. `$effect`가 `auth.isAuthenticated`가 false로 바뀐 것을 감지해서 그 시점에 `/login`으로 튕기는 거야. "진입 검증" + "상태 감시"의 이중 방어야.
...
> 💡 **onMount가 뭐야?**
> Svelte가 제공하는 라이프사이클 함수야. 컴포넌트가 DOM에 처음 붙었을 때 딱 한 번 실행돼. iOS의 `viewDidLoad()`랑 거의 같은 역할이야. 서버에서는 실행 안 되고 브라우저에서만 실행되기 때문에, API 호출 같은 클라이언트 전용 작업을 여기에 넣어.
...
> `await fetchTags()` → `await fetchMyTagIds()` 이렇게 쓰면 첫 번째가 끝나야 두 번째가 시작돼. 두 요청 시간이 **더해지는** 거야. A=200ms, B=200ms면 합계 400ms.
>
> `Promise.all([A, B])`는 두 요청을 동시에 보내고 둘 다 완료될 때까지 기다려. 각각 200ms씩 걸린다면, 순차는 400ms, `Promise.all`은 **둘 중 느린 쪽** 200ms야. 두 결과가 서로 의존하지 않을 때 쓰는 패턴이야.
...
Rust API는 두 요청을 받아서 각각 DB에 쿼리를 날려. ⑤의 `fetchTags()` 요청이 ⑦로 이어지고, ⑥의 `fetchMyTagIds()` 요청이 ⑧로 이어져. Promise.all로 동시에 발사됐으니 둘이 병렬로 진행돼.
...
이미 선택된 태그면 제거하고, 아니면 추가하는 토글이야. 여기서 핵심은 `selectedIds = next`야. 기존 Set에 바로 추가/제거하지 않고 **매 클릭마다 새 Set을 만들어서 재할당**해.

"기존 것 1개, 선택된 거 넣을 것 1개" 고정 2개가 아니야. 클릭할 때마다 새 Set 객체를 새로 생성하고, `selectedIds`에 덮어써. 이 과정을 클릭마다 반복하는 거야. [B]에서 설명한 것처럼 Svelte 5의 `$state`는 참조 비교로 변경을 감지하거든. 새 객체를 넣어야 "바뀌었다"고 인식하고 버튼 색상이 자동으로 반영돼.

"클릭마다 새 객체를 만들면 비효율적이지 않아?"라는 의문이 자연스러운데, 실제로는 괜찮아. Set에 담기는 건 태그 ID(숫자)야. 숫자는 원시값이라 복사해도 메모리 추가 점유가 거의 없어. 태그도 수십 개 수준이라 Set 하나 생성하는 비용은 체감 불가 수준이야. Svelte 반응성 규칙을 지키기 위한 불가피한 비용인데, 이 규모에서는 무시 가능해.
...
왜 기존 태그를 다 지우고 새로 넣냐면, 이유가 두 가지야.

첫째, **코드 복잡도**야. 어떤 태그가 추가됐고 어떤 게 빠졌는지 diff 계산 코드를 작성하고 유지하는 비용이 "그냥 다 지우고 다시 넣기"보다 훨씬 커. 버그 날 지점도 많아져. 그냥 전부 교체하면 돼.

둘째, **태그가 수십 개 수준이라 부담이 없어서**야. DELETE 몇 행 + INSERT 수십 행은 DB 입장에서 매우 가벼운 작업이야. 만약 수만 건의 데이터라면 달라져 — diff 계산 후 변경분만 처리하는 방식을 선택해야 해. 지금은 "규모에 맞는 단순한 전략"을 쓰는 거야.
