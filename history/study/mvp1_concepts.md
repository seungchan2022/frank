...
### 복습 자가 확인 기록 (2026-05-10, 6문제, 6/6 정답)

Q1. safeGetSession()이 getUser()까지 호출하는 이유?
사용자 답: B ✅
정답: B — getSession()은 로컬 JWT 디코드만 하기 때문에 변조된 토큰을 잡지 못함. getUser()로 Supabase Auth 서버에 실제 검증해야 확신 가능
포인트: "로컬 검증만으로는 충분하지 않다"는 보안 이유

Q2. 서버 가드([A])와 브라우저 가드(④ $effect)의 역할 차이?
사용자 답: B ✅
정답: B — 서버 가드는 페이지 진입 시 1회 실행, 브라우저 가드는 페이지가 열려있는 내내 상태 변화를 감시
포인트: 진입 검증 vs 상태 감시 — 역할이 다르기 때문에 이중 방어

Q3. 순차 await 대신 Promise.all을 쓰는 이유? (각 200ms 가정)
사용자 답: B ✅
정답: B — 순차는 400ms(합산), Promise.all은 200ms(둘 중 느린 쪽)
포인트: 두 요청이 독립적일 때 병렬 처리로 대기 시간을 절반으로 줄임

Q4. toggleTag()에서 selectedIds.add() 대신 new Set([...selectedIds])으로 재할당하는 이유?
사용자 답: B ✅
정답: B — Svelte 5의 $state는 참조 비교로 변경 감지. 같은 객체를 수정하면 화면이 다시 그려지지 않음
포인트: 매 클릭마다 새 Set 객체를 생성해서 재할당해야 반응성 동작

Q5. 태그 저장 시 기존 태그를 전부 DELETE하고 새로 INSERT하는 이유 두 가지?
사용자 답: B ✅
정답: B — diff 계산 코드 불필요(코드 복잡도 감소) + 태그가 수십 개 수준이라 전체 교체 부담 없음
포인트: 규모에 맞는 단순한 전략. 수만 건이라면 diff 방식이 맞음

Q6. onboarding_completed 업데이트에 UPSERT를 쓰는 이유?
사용자 답: C ✅
정답: C — 신규 유저는 profiles 행 자체가 없을 수 있음. UPDATE만 쓰면 행이 없을 때 아무것도 안 바뀜
포인트: 있으면 UPDATE, 없으면 INSERT — 어느 경우든 안전하게 처리
