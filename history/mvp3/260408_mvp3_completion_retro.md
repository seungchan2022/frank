...
MVP2 회고에서 제안한 "학습" 레이어가 여전히 유효하다. 이제 웹+iOS가 동일 API를 통해 데이터 일관성을 가지므로 학습 기능을 추가할 기반이 갖춰졌다:

- **스크랩 보관함** — 기사 북마크, 크로스 플랫폼 동기화
- **기사 기반 퀴즈/플래시카드** — AI 생성
- **주간 학습 리포트** — 읽은 기사 + 정답률 시각화
- **읽음 상태 동기화** — 웹/iOS 간 읽음 표시
