...
> 생성일: 260406
> 상태: ✅ 완료 (260408) — M1 / M1.5 / M2 / M3 / M4 전부 완료
> 목표: 웹과 iOS가 동일한 Rust API를 사용하여 동일한 경험 제공
> 부채: iOS 요약 60s timeout (서버는 정상 처리, 클라 UX만 깨짐) → MVP4 디스커버리 first-class 후보
...
| M2 | 웹 전환 | 웹 | ✅ 완료 (260408) | Rust API 직통 + httpOnly 쿠키 세션 전환 + 옵션 B(태그 stale 해결) — 회고: `260408_M2_회고.md` |
| M3 | iOS 전환 | iOS | ✅ 완료 (260408) | Rust API 어댑터 전환 + MVP2 부채 흡수 — 회고: `260408_M3_회고.md` |
| **hotfix** | **OpenRouter reasoning** | **서버** | **✅ 완료 (260408)** | **MiniMax M2.5 reasoning mandatory 400 fix (`5d0adc7`). M2/M3 검증 중 발견** |
| M4 | Apple 로그인 | 웹+iOS | ✅ 완료 (260408) | OAuth PKCE(웹) + ASAuthorizationController(iOS) + 크로스 플랫폼 계정 연동 확인 — 회고: `260408_M4_회고.md` |
...
```bash
cd ~/Workspace/frank
git worktree add ~/Workspace/frank-m2-web -b feature/260408_m2_web
git worktree add ~/Workspace/frank-m3-ios -b feature/260408_m3_ios
# 탭1: cd ~/Workspace/frank-m2-web && claude
# 탭2: cd ~/Workspace/frank-m3-ios && claude
```
...
4. **worktree 정리**
   ```bash
   git worktree remove ~/Workspace/frank-m2-web
   git worktree remove ~/Workspace/frank-m3-ios
