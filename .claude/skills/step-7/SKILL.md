...
## ⚠️ 무한 루프 방지 (2026-04-24 사고 기록)

**원인**: Codex가 리뷰 후 `xcodebuild test`를 반복 실행하는 루프에 빠짐 (124+ tool uses, 20분 이상).

**근본 원인 분석**:
1. Codex에 "검증하라"는 지시가 있을 때 스스로 빌드/테스트를 반복 실행하는 경향
2. 테스트 실패 → 수정 → 재테스트 사이클이 끊기지 않음
3. step-7 내에 Codex의 테스트 실행 횟수 상한이 없음

**방지 규칙 (필수 준수)**:
- Codex에는 **코드 분석/리뷰만** 요청. `xcodebuild`, `cargo test`, `npm test` 등 테스트 실행 명령 전달 금지
- Codex 프롬프트에 "실행하지 말고 분석만 해줘" 명시
- 테스트 실행은 **step-8 전용** — step-7에서 테스트 결과가 필요하면 이전 step-6 결과를 참조
- Codex 1회 호출 후 응답이 "추가 수정 필요"이면 최대 **1회만** 재호출. 그 이상은 사용자에게 보고 후 종료
