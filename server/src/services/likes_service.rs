...
/// 2. user_keyword_weights 누적 (increment_keyword_weights)
...
mod tests {
    ...
    async fn process_like_returns_keywords_and_total_likes() {
        ...
        let result = process_like(user_id, "iOS 기사", Some("Swift"), &db, &llm)
            .await
            .unwrap();
    ...
    async fn process_like_accumulates_on_multiple_calls() {
        ...
        let r1 = process_like(user_id, "기사1", None, &db, &llm)
            .await
            .unwrap();
        assert_eq!(r1.total_likes, 1);

        let r2 = process_like(user_id, "기사2", None, &db, &llm)
            .await
            .unwrap();
    ...
    async fn process_like_no_snippet() {
        ...
        let result = process_like(user_id, "title only", None, &db, &llm)
            .await
            .unwrap();
