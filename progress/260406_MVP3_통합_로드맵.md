...
| M# | 마일스톤 | 범위 | 내용 |
|----|---------|------|------|
| M1 | API Contract | 서버 | Rust 서버에 부족한 엔드포인트 추가 + 데이터 모델 통일 |
| M2 | 웹 전환 | 서버+웹 | Supabase 직접 호출 → Rust API + @supabase/ssr 쿠키 전환 |
| M3 | iOS 전환 | 서버+iOS | Supabase 직접 호출 → Rust API + MVP2 부채 해소 |
| M4 | Apple 로그인 | 서버+웹+iOS | 3개 플랫폼 동시 구현 (참조: wip/apple-login-draft 2fbcedc) |
