...
- [x] F-12 웹 설정 페이지 occupation 입력란 추가 + PATCH 저장 ← ST-5 완료
- [x] F-13 웹 요약하기 — insight 있으면 표시, null이면 섹션 숨김 ← ST-6 완료
- [x] F-14 웹 재작성 버튼 — occupation 설정 시에만 렌더링 ← ST-6 완료
...
- [x] E-05 요약하기 + 재작성 동시 호출 시 각각 독립 처리 ← 서버 핸들러 독립 구현으로 충족

### 에러
- [x] R-01 Groq 호출 timeout (10s) 시 503 반환 ← 기존 Groq 어댑터 구현으로 충족
- [x] R-02 Groq 1회 retry 후에도 실패 시 503 반환 ← 기존 Groq 어댑터 구현으로 충족
- [x] R-03 에러 응답에 occupation 내용 노출 금지 (에러 메시지는 occupation_required 고정 문자열)
- [x] R-04 DB occupation 조회 실패 시 500 반환 ← 400 반환으로 설계 변경, degrade 없음으로 해소
...
- [x] T-04 웹 vitest — occupation 입력/저장 동작, 재작성 버튼 숨김 조건 ← ST-5/6 완료 (267 tests)
- [x] T-05 iOS xcodebuild test — SettingsFeature occupation, ArticleDetailFeature rewrite 시나리오 ← ST-7/ST-8 완료 (273 tests passed)
- [x] T-06 본인 E2E — 직업 설정 → 요약하기 인사이트 표시 1회 확인 ← 브라우저 검증 완료 (2026-05-03)
- [x] T-07 본인 E2E — 직업 설정 → 재작성 1회 확인 ← 브라우저 검증 완료 (2026-05-03)

### UI·UX
- [x] U-01 웹 insight 섹션 — null 시 레이아웃 깨짐 없음 확인 ← ST-6 브라우저 검증 완료
- [x] U-02 iOS insight 섹션 — nil 시 섹션 숨김 처리 구현 완료 (E2E 시뮬레이터 검증 사용자에게 위임)
- [x] U-03 재작성 로딩 중 버튼 비활성화 ← handleRewrite() loading/done guard로 구현 완료
