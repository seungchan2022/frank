...
워크플로우(step-1~step-9) 진행 중 반드시 준수한다.

### 9.1 스텝 헤더 (매 응답 필수)

워크플로우 진행 중 모든 응답 **첫 줄**에 반드시 표시한다:
```
[Step N/9 — /step-N | 인터뷰 M/T 완료]   ← 인터뷰 진행 중
[Step N/9 — /step-N]                      ← 그 외
```

### 9.2 인터뷰 완료 게이트

step-1, step-4 인터뷰 진행 중:
- **모든 질문 완료 전 다음 스텝 전환 절대 금지**
- 각 질문마다 `Q{현재}/{총}` 카운터 표시 (예: `Q2/4`)
- 인터뷰 시작 시 총 질문 수 사전 고지
- 사용자가 중간에 다른 논의를 요청해도 논의 후 반드시 인터뷰로 복귀: `"Q{M}/{T}로 돌아갑니다"` 명시
- 마지막 답변 후 `"인터뷰 완료 (T/T)"` 선언 후에만 다음 단계 진행

### 9.3 피드백 수신 후 구현 진입 규칙

사용자가 테스트 결과·버그 보고·검증 피드백을 전달했을 때:
1. **즉시 구현 금지** — 수정 방향을 분석·제시한다
2. **"이 방향으로 수정할까요?"** 확인 후 구현 진입
3. 설계 변경이 필요한 경우 step-1 또는 step-3으로 복귀해 범위 재정의

### 9.4 서브에이전트 실행 알림

병렬 에이전트(Codex, Serena 등) 호출 전 반드시 알린다:
```
[에이전트 실행 중] Codex 리뷰 시작 — 완료까지 수 분 소요될 수 있습니다.
```
- 30초 이상 소요 예상 시 사전 안내 필수
- 응답 없이 2분 이상 경과 시 사용자에게 "대기 중" 상태 알림
