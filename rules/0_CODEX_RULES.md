...
본 저장소의 **강제 워크플로우는 §9의 9단계 (step-1~step-9)** 다. 세부 절차는 `.claude/skills/workflow/SKILL.md`를 참조한다.

> 참고: Codex 호출 시 자동 작동하는 내부 사고 모델은 **5단계** (Inspect → Specify → Implement → Verify → Report)이며, 9단계의 step-2/5/7 안에서 자연스럽게 충족된다. 별도로 의식할 필요 없음.
...
* `rules/sub/workflow.md` — 개발 사고 가이드 (9단계 워크플로우 내부 참고용)
...
## 9. 워크플로우 진행 강제 규칙 (Workflow Enforcement)

본 저장소의 강제 워크플로우는 **9단계 (step-1~step-9)**. 강제 룰의 **SSOT는 `.claude/skills/workflow/SKILL.md`** 이며, 본 §9는 게이트의 **존재**만 명시한다 — 세부 형식·예외·복구 절차는 SSOT 참조.

### 9.1 핵심 게이트 (요약)

- **스텝 헤더**: 매 응답 첫 줄에 `[Step N/9 — /step-N | 인터뷰 M/T 완료]` 또는 `[Step N/9 — /step-N]` 표시 필수
- **인터뷰 완료 게이트**: step-1/step-4 인터뷰 모든 질문 완료 전 다음 스텝 전환 금지. `Q{M}/{T}` 카운터 필수
- **피드백 수신 후 구현 진입**: 사용자 피드백 수신 즉시 구현 금지. 수정 방향 제시 → 확인 → 구현
- **서브에이전트 실행 알림**: 30초 이상 소요 에이전트(Codex, Serena 등) 호출 전 사전 안내 필수

각 게이트의 정확한 형식·예외·복구 절차는 `.claude/skills/workflow/SKILL.md` 참조.
