...
| codex | **테스트 실행 명령 전달 금지** — `xcodebuild`, `cargo test`, `npm test` 등 빌드/테스트 명령을 Codex 프롬프트에 포함하거나 실행하도록 지시 금지. 테스트 실행은 step-8 전용. | 무한 루프 방지 (2026-04-24 사고) |
| codex | **재호출 최대 1회** — 1회 호출 후 "추가 수정 필요" 응답이면 1회만 재호출. 초과 시 사용자에게 보고 후 종료. | 무한 루프 방지 (2026-04-24 사고) |
| codex | Codex 프롬프트에 **"실행하지 말고 분석만 해줘"** 명시 필수 | 무한 루프 방지 (2026-04-24 사고) |
